let requestCount = 0;
let GROQ_API_KEY = "gsk_сіздің_кілтіңіз"; // Осы жерге GroqCloud кілтін қойыңыз
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                errorMsg = "API ключі қате. Кілтіңізді тексеріңіз";
            } else if (response.status === 429) {
                errorMsg = "Лимиттен асып кеттіңіз. Кейінірек қайталаңыз";
